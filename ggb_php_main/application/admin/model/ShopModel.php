...
class ShopModel extends Model
{
    ...
    {
        return $this->where($where)->limit($offset, $limit)->order('store_id desc')->select();
    }
...
//                $result = $this->validate('ProductValidate')->save($param);
    ...
    {
        try{
            $result = $this->validate('ProducttitleValidate')->save($param, ['store_id' => $param['store_id']]);
            if(false === $result){
                // 验证失败 输出错误信息
                return msg(-1, '', $this->getError());
            }else{
                $res = Db::name('shop')->where('store_id',$param['store_id']) ->update(['store_static' => '2']);
                return msg(1, url('shop/shop'), '审核不通过成功');
            }
        }catch(\Exception $e){
            return msg(-2, '', $e->getMessage());
        }
     ...
     * @param $id
     */
    public function getOneProduct($store_id)
    {
     ...
     * @param $id
     */
    public function delProduct($store_id)
    {
        try{
