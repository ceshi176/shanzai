...
use think\Db;
class NoauditModel extends Model
{
    // 确定链接表名
    ...
    {
          $arr=Db::name('others')->alias('o')
            ->join('shop s','o.store_id = s.store_id','left')
            ->field('s.store_name,o.id,o.license,o.identity,o.identity_s,o.door,o.audit,o.store_stac,o.registration,o.other,o.organization')
            ->where($where)
            ->limit($offset, $limit)
            ->order('o.id desc')
            ->select();
    ...
    public function editProduct($param)
    {
        try{
            ...
            if(false === $result){
                ...
                $res = Db::name('others')->where('id',$param['id']) ->update(['audit' => '3']);
                return msg(1, url('noaudit/noaudit'), '审核不通过成功');
            }
        }catch(\Exception $e){
